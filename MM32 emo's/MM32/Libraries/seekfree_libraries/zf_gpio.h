...
typedef enum																					// 枚举端口方向
{
	GPI_AIN = 0x00,																				// 定义管脚模拟输入
	GPI_FL = 0x04,																				// 定义管脚浮空输入
	GPI_PD = 0x08,																				// 定义管脚下拉输入
	GPI_PU = 0x08,																				// 定义管脚上拉输入
	GPO_PP = 0x03,																				// 定义管脚推挽输出
	GPO_OD = 0x07,																				// 定义管脚开漏输出
	GPO_AFPP = 0x0B,																			// 定义管脚复用推挽输出
	GPO_AFOD = 0x0F,																			// 定义管脚复用开漏输出
}GPIOMODE_enum;
